...
Rewrite of forked script `naive-hashcat` which modifies the way to specify arguments, gives some log messages for progress tracking, and allow to specify rules and masks files in bash arrays (automatically used in a loop).

Only main script has been kept and updated.

Tested with hashcat-5.1.0.
...
4 strategies:
   - 1/4 Dictionnary
   - 2/4 Dictionnary + Rules (if any in corresponding array)
   - 3/4 Masks (if any in corresponding array)
   - 4/4 Combinatorial (disabled in source because very intensive)


### How to setup

```
git clone https://github.com/rfenouil/naive-hashcat.git
```

Use text editor on `naiveH.sh` script and edit following lines

```
# Path to hashcat binary
HASHCAT_BIN=${HASHCAT_BIN:-"$HOME/hashcat-5.1.0/hashcat64.bin"}
# Bash arrays for rules and masks files
HASHCAT_RULEFILES=( "$HOME/hashcat-5.1.0/rules/d3ad0ne.rule" "$HOME/hashcat-5.1.0/rules/rockyou-30000.rule" )
HASHCAT_MASKFILES=( "$HOME/hashcat-5.1.0/masks/rockyou-1-60.hcmask" )
...
```
naiveH.sh <pathToHashFile> <pathToDicFile> <hashTypeCode>
```
